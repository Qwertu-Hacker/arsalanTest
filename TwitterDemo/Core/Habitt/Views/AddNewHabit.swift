        ...
        NavigationView {
            VStack(spacing: 15) {
                TextField("Title", text:  $habitModel.title)
                    .padding(.horizontal)
                    .padding(.vertical)
                    .background(Color.gray.opacity(1), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
                HStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Frequency")
                        .font(.callout.bold())
                    let weekDays = Calendar.current.weekdaySymbols.dropFirst() + [Calendar.current.weekdaySymbols.first!]
                    HStack(spacing: 10) {
                            ...
                            Text(day.prefix(2))
                                ...
                                .background {
                                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                                        .fill(index != -1 ? Color(habitModel.habitColor) : Color.gray.opacity(1))
                                }
                                .onTapGesture {
                                    withAnimation {
                                        if index != -1 {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarLeading) {
                    Button {
                        if habitModel.deleteHabit(content: done.managedObjectContext) {
                            done.dismiss()
                        }
                    } label: {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button {
                        Task {
                        if await habitModel.addHabit(context: done.managedObjectContext) {
                            done.dismiss()
                            }
                        }
                    } label: {
